...
**👀 Observation:** Web Inspector (DevTools) works a little different than, for example, in Chrome: even with the **Show features for web developers** checkbox in Safari checked, there's no **Develop** menu item nor can you right-click and **Inspect Element** in a web app. Instead, you debug apps via Safari's **Develop > $machineName > $appName** menu item (thanks, [@samedwards@mastodon.social](https://mastodon.social/@samedwards/110504651650657859)). If all app windows are closed but the app isn't quit, a potential service worker will be inspectable until it gets terminated after a couple of seconds.
